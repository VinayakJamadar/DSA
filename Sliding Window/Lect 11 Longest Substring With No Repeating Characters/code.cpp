...
// 5. If DS2 is less than k
//      5.1. Iterate till DS2 is less than k
...
int main(){
    ...
    while(j < n) {
        ...
        // 4. If DS2 equal to k
        if(alphabetCnt.size() == j-i+1) {
            // 4.1. Find the answer from DS1 and DS2 and store on DS1
            ans = max(ans, j-i+1);
        }
        // 5. If DS2 is less than k
        else if(alphabetCnt.size() < j-i+1) {
            // 5.1. Iterate till DS2 is less than k
            while(alphabetCnt.size() < j-i+1) {
                // 5.1.1. If Requried Remove Calculations done on ith element from DS2
                alphabetCnt[str[i]]--;
    ...
    // 7. Displaying or Returning the DS1
    cout<<"Longest Substring With No Repeating Characters is : "<<ans;
    return 0;
}