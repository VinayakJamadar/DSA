...
//          c. Either ind1 or ind2 stored the index of Searched Element 'elem'.

#include<bits/stdc++.h>
using namespace std;

int BS(vector<int> arr, int low, int high, int elem) {
    int ind = -1;
    while(low <= high) {
        int mid = (low + high)/2;
        if(arr[mid] == elem) {
...
int main(){
    ...
    int n = arr.size(), low = 0, high = n - 1, minElemInd = 0, elem = 15;
    while(low <= high) {
        int mid = (low + high)/2;
        int prev = (mid+n-1)%n;
        if(arr[mid] <= arr[prev]) {
    ...
    if(minElemInd == 0) {
        int ind = BS(arr, 0, n-1, elem);
        if(ind != -1) cout<<"Element is present at index : "<<ind;
        else cout<<"Element not present in array.";
    }
    else {
        int ind1 = BS(arr, 0, minElemInd-1, elem);
        int ind2 = BS(arr, minElemInd, n-1, elem);
